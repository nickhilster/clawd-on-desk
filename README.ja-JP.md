...
<p align="center">
  <img src="assets/hero.gif" alt="DeskBuddy のアニメーションデモ。AI コーディングエージェントの状態に合わせて、デスクトップペットが睡眠、思考、ツール実行中のタイピング、サブエージェント 1 個ではヘッドホングルーヴ、複数では3ボールジャグリング、権限リクエストの通知、タスク完了後のお祝いへリアルタイムに切り替わります。Claude Code、Codex、Cursor、Copilot、Gemini、Antigravity、Qwen、Pi、OpenClaw などに対応しています。">
</p>

DeskBuddy はデスクトップに住むペットで、AI コーディングエージェントが今何をしているかにリアルタイムで反応します。長いタスクを開始したら席を外し、DeskBuddy が完了を知らせたら戻ってくるだけです。
...
- **Gemini CLI** — `~/.gemini/settings.json` の command hook に対応（DeskBuddy 起動時に自動登録、または `npm run install:gemini-hooks`）
- **Antigravity CLI (agy)** — `~/.gemini/config/hooks.json` の command hook に対応（Antigravity config がある場合は DeskBuddy 起動時に自動登録、または `npm run install:antigravity-hooks`）。**state-only** のため、DeskBuddy は agy の権限バブルを表示しません。Allow / Deny / Always-allow は agy 自身のターミナルメニューで選択します
- **Cursor Agent** — `~/.cursor/hooks.json` の [Cursor IDE hooks](https://cursor.com/docs/agent/hooks) に対応（DeskBuddy 起動時に自動登録、または `npm run install:cursor-hooks`）
- **CodeBuddy** — `~/.codebuddy/settings.json` 経由で Claude Code 互換の command hook と HTTP permission hook に対応（DeskBuddy 起動時に自動登録、または `node hooks/codebuddy-install.js`）
- **Kiro CLI** — `~/.kiro/agents/` 配下のカスタムエージェント設定に command hook を注入。DeskBuddy 起動時には Kiro 組み込みの `kiro_default` から再同期される `deskbuddy` エージェントも自動作成されるため、`kiro-cli --agent deskbuddy` または `/agent swap deskbuddy` で挙動差を抑えながら hook を利用できます（DeskBuddy 起動時に自動登録、または `npm run install:kiro-hooks`）。state hook は macOS と Windows で検証済みです。
- **Kimi Code CLI (Kimi-CLI)** — `~/.kimi/config.toml` の command hook（`[[hooks]]` エントリ）に対応（DeskBuddy 起動時に自動登録、または `npm run install:kimi-hooks`）
- **Qwen Code** — `~/.qwen/settings.json` の command hook に対応（DeskBuddy 起動時に自動登録、または `npm run install:qwen-hooks`）。状態追跡と Qwen `PermissionRequest` のデスクトップ権限バブルに対応します
- **opencode** — `~/.config/opencode/opencode.json` 経由の [plugin integration](https://opencode.ai/docs/plugins) に対応（DeskBuddy 起動時に自動登録）。遅延のないイベントストリーミング、Allow/Always/Deny 付きの権限バブル、`task` tool による並列サブエージェント生成時の building アニメーションに対応
- **Pi** — `~/.pi/agent/extensions/deskbuddy` のグローバル extension で連携します（DeskBuddy 起動時に自動登録、または `npm run install:pi-extension`）。インタラクティブな Pi セッションのライフサイクルとツール活動だけを状態同期し、Pi のデフォルト YOLO 動作を維持します
- **OpenClaw** — `~/.openclaw/openclaw.json` の plugin path で state-only 連携します（OpenClaw config が既にある場合は DeskBuddy 起動時に自動登録、または `npm run install:openclaw-plugin`）。Phase 1 はローカル `openclaw tui --local` セッションのアニメーションのみを対象とし、権限バブルやターミナルフォーカスには対応しません
- **Hermes Agent** — Hermes の管理 plugin ディレクトリ経由の [plugin integration](https://hermes-agent.org/)（Hermes インストール済みの場合は DeskBuddy 起動時に自動登録、または `npm run install:hermes-plugin`）。状態、セッション、SessionEnd、ターミナルフォーカスに対応
- **Qoder** — `~/.qoder/settings.json` の command hook で state-only 連携します（`~/.qoder/` ディレクトリがある場合は DeskBuddy 起動時に自動登録、または `npm run install:qoder-hooks`）。Phase 1 はアニメーションのみを対象とし、権限リクエストは通知として観察するだけで、DeskBuddy は権限バブルを表示せず代わりに応答もしません。Allow / Deny は Qoder 自身の権限フローで選択します
- **複数エージェントの共存** — すべてのエージェントを同時に動かせます。DeskBuddy は各セッションを個別に追跡します

### アニメーションと操作
- **リアルタイムな状態認識** — agent hook とログポーリングが DeskBuddy のアニメーションを自動で切り替えます
...
- **Codex Pet インポート** — `Settings...` → `Theme` から Codex Pet zip パッケージをインポートすると、DeskBuddy が atlas アニメーションを管理テーマに変換します
- **視線追従** — idle 状態では DeskBuddy がカーソルを追い、体の傾きや影の伸びも変化します
- **睡眠シーケンス** — 60 秒アイドルが続くと、あくび、うとうと、倒れ込み、睡眠へ移行します。マウス移動で驚いて起きるアニメーションが再生されます
- **クリックリアクション** — ダブルクリックでつつき、4 回クリックでじたばたします
- **どの状態からでもドラッグ** — いつでも DeskBuddy をつかめます（Pointer Capture により素早いフリックでも取り落としません）。離すと元の状態へ戻ります
- **Mini mode** — 右端へドラッグ、または右クリックの「Mini Mode」で有効化。DeskBuddy が画面端に隠れ、ホバーで顔を出し、mini 通知やお祝い、放物線ジャンプの遷移を行います

### 権限バブル
- **アプリ内権限レビュー** — Claude Code、Codex CLI、CodeBuddy、opencode がツール権限を要求すると、ターミナルで待つ代わりに DeskBuddy がフローティングバブルカードを表示します
...
- **Sessions dashboard + HUD** — 右クリックまたは tray → `Open Dashboard` から live session、最近のイベント、エイリアス、ターミナルジャンプを確認できます。DeskBuddy の近くにはコンパクトな HUD が表示され、現在の live session を見失いません
...
- **起動時リカバリー** — DeskBuddy の再起動時に対応エージェントがまだ動いていれば、眠らずに起きたままになります

### モバイルコンパニオン（PWA）
- **スマホでライブミラー** — `Settings...` → `Mobile / PWA` を有効にし、スマホでペアリング URL を開くと、DeskBuddy Mobile ウェブアプリがエージェントのセッションと状態をリアルタイムに表示します
...
> モバイルコンパニオンのラインは、最初のプロトタイプからトークンローテーションまで、コアコントリビューター [@Bynlk](https://github.com/Bynlk) が作り上げ、リードしています。ネイティブ Android アプリを備えた姉妹プロジェクト [deskbuddy-mobile](https://github.com/Bynlk/deskbuddy-mobile) も同氏がメンテナンスしています。

### システム
- **クリック透過** — 透明部分のクリックは背面ウィンドウへ通り、DeskBuddy の体だけが操作対象になります
- **位置の記憶** — 再起動後も最後に置いた場所を覚えます（mini mode を含む）
- **単一インスタンスロック** — DeskBuddy ウィンドウの重複起動を防ぎます
- **自動起動** — Claude Code の SessionStart hook により、DeskBuddy が起動していない場合に自動で起動できます
...
DeskBuddy はマルチモニター環境に適応します。起動したディスプレイに合わせて比例サイズを決め、縦長モニターでは読みやすさを保つために上限付きで拡大し、ディスプレイ間のドラッグにも対応します。
...
インストール後に DeskBuddy を起動してください。対応エージェントの hook/plugin は起動時に自動同期されます。
...
# DeskBuddy を起動（起動時に Claude Code hooks を自動登録）
npm start
```

**Claude Code**、**Codex CLI**、**Copilot CLI** は自動登録される hook により、そのまま動作します。**Gemini CLI**、**Antigravity CLI (agy)**、**Cursor Agent**、**CodeBuddy**、**Kiro CLI**、**Kimi Code CLI (Kimi-CLI)**、**Qwen Code**、**opencode**、**Pi**、**OpenClaw**、**Hermes Agent**、**Qoder** は、インストール済みかつ初期化済みであれば DeskBuddy 起動時に自動登録されます。Remote SSH、WSL、macOS/Linux のプラットフォーム別メモも含めた詳細: **[docs/guides/setup-guide.md](docs/guides/setup-guide.md)**

リモートサーバーで Claude Code / Codex CLI を動かし、状態と権限バブルをローカル DeskBuddy に転送したい場合は、アプリ内の **Settings → Remote SSH → One-click deploy** を使います。完全な手順、Doctor の境界、FAQ はこちら: **[docs/guides/guide-remote-ssh.md](docs/guides/guide-remote-ssh.md)**

公式の `Codex + WSL` ステータス、DeskBuddy の現在の実装境界、そして誤解しやすい理由については、こちらを参照してください: **[docs/guides/codex-wsl-clarification.md](docs/guides/codex-wsl-clarification.md)**
...
DeskBuddy はカスタムテーマに対応しています。標準キャラクターを、自分のキャラクターやアニメーションに置き換えられます。既存の Codex Pet パッケージがある場合は、`Settings...` → `Theme` → `Import pet zip` から取り込むと、DeskBuddy が atlas を管理テーマへ自動変換します。
...
3. DeskBuddy を再起動するか、`Settings...` → `Theme` → 自分のテーマを選択します
...
DeskBuddy をより良くしてくれたすべての方に感謝します。
