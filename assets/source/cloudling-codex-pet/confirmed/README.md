...
| 状态 | 推荐文件 | 说明 |
|---|---|---|
| mini-crabwalk | `states/mini-crabwalk-soft-scoot-v1.svg.html` | 右键进入 Mini Mode 前的边缘小挪步确认版。运行时由 deskbuddy `animateWindowX` 移动窗口到屏幕边缘，SVG 内部只做原地软弹赶路循环；云宝小幅 scoot / hop / squash + 轻滚角，眼睛使用 long-idle 同款正常胶囊眼 ↔ 稍宽眼 `widenFactor`，身后三团小云尘在浅色模式下带淡蓝描边/填色以保持可读。|
