...
远程 SSH 的默认入口是 DeskBuddy 应用内的 **Settings -> 远程 SSH**。旧的
...
DeskBuddy 不保存 SSH 密码或私钥口令。首次 host key 确认、passphrase、ssh-agent
加载都交给系统 `ssh` 和系统终端处理。
...
3. 如果 SSH 需要首次确认 host key、输入 passphrase 或加载 ssh-agent，点 **首次认证**。DeskBuddy 会打开系统终端运行一次普通 `ssh`。
4. 点击 **一键部署**。
   - DeskBuddy 会先打开并维护 `ssh -R` 反向隧道
   - 然后从当前已安装的 DeskBuddy 应用复制 hook 文件到远端 `~/.claude/hooks/`
...
远端 Claude/Codex/Copilot hook
  ...
  -> SSH 反向隧道
  -> 本机 DeskBuddy http://127.0.0.1:<本地运行端口>
