...
3. Whether DeskBuddy can currently auto-detect Codex sessions that live inside WSL's separate Linux home.

Those answers are not the same. Most confusion comes from mixing the official Codex product support story with DeskBuddy's current integration boundaries.
...
- DeskBuddy now uses Codex official hooks as the primary integration and keeps `~/.codex/sessions` JSONL polling as fallback.
...
- When DeskBuddy runs on Windows while Codex runs inside WSL with the default Linux home directory, DeskBuddy does not automatically see `/home/<user>/.codex/sessions`.
- So "Codex does not support WSL" is inaccurate. The more accurate statement is: "Codex officially supports WSL2, but Windows DeskBuddy does not automatically modify or poll WSL's separate Linux `~/.codex`; install hooks in WSL remote mode or share `CODEX_HOME` when you want WSL Codex sessions to report back."
...
DeskBuddy's Codex installer writes `~/.codex/hooks.json` and enables this feature flag unless the user explicitly set hooks to `false`. If it sees the deprecated `codex_hooks` key, it migrates it to `hooks` while preserving an explicit false value.
...
## 2. DeskBuddy's current implementation

### DeskBuddy uses official hooks first, with JSONL polling as fallback
...
- PermissionRequest defaults to intercept mode: DeskBuddy shows a real Allow/Deny bubble. Users can switch Codex permission mode to native if they want Codex AutoReview/native prompts to stay in charge.
...
- if DeskBuddy runs on Windows, it polls `C:\Users\<user>\.codex\sessions`
- if DeskBuddy runs on Linux, it polls `/home/<user>/.codex/sessions`

So a Windows-hosted DeskBuddy does not automatically poll `/home/<user>/.codex/sessions` inside WSL.

### DeskBuddy currently has no automatic "install into WSL home" path
...
- `Codex native on Windows + DeskBuddy on Windows`: works out of the box through official hooks, with JSONL fallback
- `Codex in WSL2 + DeskBuddy on Windows + default Linux home`: does not work by default unless hooks are installed inside WSL in remote mode or a remote-side monitor is started
...
The last point is an inference from OpenAI's documented `CODEX_HOME` sharing flow plus DeskBuddy's current hook and fallback polling behavior.

### DeskBuddy has remote-side hooks and a monitor fallback, but not WSL auto-discovery

This repo includes remote deployment through [`scripts/remote-deploy.sh`](../../scripts/remote-deploy.sh). It copies Codex hook files, runs `node ~/.claude/hooks/codex-install.js --remote`, and registers official Codex hooks that POST back through an SSH reverse tunnel with `DESKBUDDY_REMOTE=1`.
...
3. DeskBuddy uses official Codex hooks as primary integration and JSONL polling as fallback.
4. DeskBuddy currently syncs hooks and polls fallback logs in the host machine's own `.codex` home.
5. So if Codex runs inside WSL2 and still uses Linux `~/.codex`, Windows DeskBuddy does not auto-detect those sessions by default.
...
> OpenAI officially supports running Codex in WSL2. DeskBuddy integrates with Codex through official hooks, with JSONL polling kept as fallback. If DeskBuddy runs on Windows while Codex runs in WSL with Linux's default home directory, DeskBuddy will not automatically modify or poll that WSL `~/.codex`; install hooks in WSL remote mode, share `CODEX_HOME`, or run the remote fallback monitor. The current issue is better described as a DeskBuddy integration and documentation boundary, not as OpenAI failing to support Codex on WSL.

## 6. Current workable paths

If the goal is "make Windows DeskBuddy react to Codex running in WSL", the realistic paths today are:

1. Use OpenAI's documented sharing approach and point WSL `CODEX_HOME` at the Windows `%USERPROFILE%\.codex`.
2. Install Codex official hooks inside WSL with `node hooks/codex-install.js --remote`, so WSL Codex POSTs back to Windows DeskBuddy through WSL localhost forwarding.
...
4. Extend DeskBuddy itself to support an explicit `\\wsl$\...` session directory.

Those options are not equivalent:

- option 1 is an OpenAI-documented sharing flow
- option 2 is the lowest-latency DeskBuddy path when Codex hooks are available inside WSL
- option 3 is the fallback for hook-disabled Codex sessions
- option 4 would be a new DeskBuddy feature, not current default behavior
